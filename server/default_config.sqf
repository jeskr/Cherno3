...
A3W_timeMultiplierNight = 30;     // Sets the speed of time between 8 PM and 5 AM
...
// extDB settings
A3W_extDB_ServerID = 3;            // Server ID to use in the database for the particular server running off this config file; if you have multiple servers, they all need different IDs
...
A3W_mainMissionTimeout = 90*60;    // Time in seconds that a Main Mission will run for, unless completed
...
A3W_moneyMissionTimeout = 90*60;   // Time in seconds that a Money Mission will run for, unless completed
